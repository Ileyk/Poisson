...
def rho(it):
    ...
    N2=42
    N=N1*N2
    # +1 in order to plot also the last row and column
    # BEWARE, WORKS ONLY FOR 1 GHOST CELL (i.e. pencil=3)
    w=np.zeros((N2+1,N1+1))
    f=np.zeros((N2+1,N1+1))
    fth=np.zeros((N2+1,N1+1))
    r=np.zeros((N2+1,N1+1))
    x=np.zeros(N1+1)
    y=np.zeros(N2+1)
    ...
    x[N1]=x[N1-1]+(x[N1-1]-x[N1-2]) # dumb last coordinate
    y[N2]=y[N2-1]+(y[N2-1]-y[N2-2]) # dumb last coordinate
    file.close()
    xmin=np.min(x)
    xmax=np.max(x)
    ymin=np.min(y)
    ...
    for i in range(N1):
        ...
        w[N2,i]=0. # dumb last value
    w[N2,N1]=0. # dumb last value
    file.close()
    ...
    for i in range(N1):
        ...
        fth[N2,i]=0. # dumb last value
    fth[N2,N1]=0. # dumb last value
    file.close()
    ...
    for it in range(int(start),int(end)+1,int(step)):
        ...
        for i in range(N1):
            ...
            f[N2,i]=0. # dumb last value
        f[N2,N1]=0. # dumb last value
        file.close()
        ...
        for i in range(N1):
            ...
            r[N2,i]=0. # dumb last value
        r[N2,N1]=0. # dumb last value
        file.close()
        ...
        cax=ax.pcolormesh(X,Y,f,cmap='gist_rainbow',norm=Normalize(vmin=-0.25,vmax=0.25))
        cbar=plt.colorbar(cax,ax=ax)
        ax.set_xlim(xmin-0.05*Lx,xmax+0.05*Lx)
        ax.set_ylim(ymin-0.05*Ly,ymax+0.05*Ly)
        ...
        # plt.subplots_adjust(wspace=0.,hspace=0.2)
        cax=ax.pcolormesh(X,Y,(f-fth)/np.abs(fth),norm=SymLogNorm(linthresh=1.E-6,linscale=1.E-6,vmin=-1E0,vmax=1E0),cmap='seismic')
        cbar=plt.colorbar(cax,ax=ax)
        ax.set_xlim(xmin-0.05*Lx,xmax+0.05*Lx)
        ax.set_ylim(ymin-0.05*Ly,ymax+0.05*Ly)
        # ax.set_ylim(-1.1*np.max(np.abs(fth)),1.1*np.max(np.abs(fth)))
        ...
        cax=ax.pcolormesh(X,Y,r,norm=SymLogNorm(linthresh=1.E-6,linscale=1.E-6,vmin=-1E0,vmax=1E0),cmap='seismic')
        cbar=plt.colorbar(cax,ax=ax)
        ax.set_xlim(xmin-0.05*Lx,xmax+0.05*Lx)
        ax.set_ylim(ymin-0.05*Ly,ymax+0.05*Ly)
